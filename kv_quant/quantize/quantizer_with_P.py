...
def k_quant_per_token(
    ...
    x_fp_new = (
        x_fp_new.permute(0, 2, 1, 3).view(batch, seq_len, num_head * h_dim).contiguous()
    ).to(torch.float16)
...
def k_quant_per_channel(
    ...
    batch, num_head, seq_len, h_dim = x_fp.shape
    x_fp_new = (
        x_fp.permute(0, 2, 1, 3).view(batch, seq_len, num_head * h_dim).contiguous()
    )
...
def p_quant_per_block(
    ...
    x_fp = x_fp.view(torch.float16)

    if q_bits >= 16:
        return x_fp
    
    if q_bits == 12:
        x_fp = x_fp.view(torch.int16)
        x_fp_lsb = x_fp.bitwise_and(7)
        x_fp = x_fp.bitwise_and(65528)
        roundup_mask = x_fp_lsb.gt(3)
        x_fp[roundup_mask] = x_fp[roundup_mask] + 8
        return x_fp.view(torch.float16)
...
def quant_matmul_pv(
    ...
    v_int = v_int.view(-1, -1, -1, num_groups_per_head, v_group_size).transpose(2, 3).contiguous() # batch, num_head, num_groups_per_head, kv_seq_len, group_size
    ...
    p_dq = p_quant_per_block(p_fp, quant_config.p_bits) # batch, num_head, num_groups_per_head, q_seq_len, kv_seq_len
    attn_output = torch.matmul(p_dq, v_int) # batch, num_head, num_groups_per_head, q_seq_len, group_size
    attn_output = attn_output.transpose(2, 3).view(-1, -1, -1, h_dim).contiguous() # batch, num_head, q_seq_len, h_dim
