        ...
        if not self.kv_quant_post_attn:
            if past_key_value is None: # Prefill Stage
                ...
                if value_states_float_len > self.kv_residual_len:
                    assert value_states_float_len == self.residual_length + 1, \
                    ...
                    assert value_states_float.shape[-2] == self.residual_length, \
                        f"Wrong value_states_float_len !"
                
                ######################################## Q x K.T ########################################
                key_states_full = torch.cat([key_states_quant, key_states_quant_new], dim=2)
